...
### AI Tag Generation During Organization

`organize` and `preview` can ask the text/vision model to attach tags to each file as part of the
same analysis pass, recorded per-file in the resulting plan. This is a separate mechanism from the
heuristic `autotag` command family above — it costs one extra model round-trip per file and the
tags come from the AI's own reading of the content, not from local heuristic scoring.

```bash
# Generate tags for every organized file
file-organizer preview ~/Downloads --generate-tags

# Nudge the model toward a tagging domain (same five presets as `autotag`: sfx, audio, code,
# descriptive, hierarchical) and/or give it free-text guidance
file-organizer preview ~/Downloads --generate-tags --tag-style descriptive --tag-prompt "focus on client names and project codes"

# Same flags apply to organize; on organize they're advanced options (see --advanced-help)
file-organizer organize ~/Downloads ~/Organized --generate-tags --tag-style code
```

`--tag-style` here reuses the same five preset names as `autotag`, but the effect is different:
instead of a scoring boost, it adds a "favor terms fitting the '`<style>`' domain" hint to the
model's prompt. `--tag-prompt` similarly becomes free-text guidance appended to the prompt (also
capped at 500 characters). Both require `--generate-tags`; setting either without it is rejected.
If you pass `--plan` to `organize` with a previously saved reviewed plan, omitting these flags
inherits the plan's own tag settings — only flags you repeat on the command line override it.
...
The auto-tagging system suggests and applies tags to files. It uses heuristic analysis of
filename, extension, directory, metadata, and content signals — no model call required. For
tags generated by the AI model itself during `organize`/`preview`, see
[AI Tag Generation During Organization](#ai-tag-generation-during-organization) above.
...
### Style Presets and Custom Prompts

`autotag suggest` and `autotag batch` accept `--style`/`-s` and `--prompt`/`-p` to steer which
candidate tags score highest. Both flags are heuristic-only — they rerank and combine tags already
extracted from filename, extension, directory, metadata, and content signals; they don't call an
AI model.
...
file-organizer autotag batch ~/Projects --style code --prompt "backend services"
```

The five presets:

- **`sfx`** (alias: **`audio`**) — doubles the score of any candidate tag found in a ~40-term
  sound-design vocabulary (`whoosh`, `impact`, `ambient`, `foley`, `riser`, `drone`, `reverb`,
  `synth`, and similar terms). `sfx` and `audio` share this exact vocabulary and boost — pick
...
- **`hierarchical`** — emits compound `<category>/<subcategory>` tags by pairing each
  extension-derived tag with each directory- or content-derived tag (e.g. a `.pdf` in a
  `contracts/` folder can produce `pdf/contracts`), for building a nested tag taxonomy.

`--prompt` applies independently of `--style` and works with every preset (or with no preset at
all): each word in the prompt (split on spaces, hyphens, underscores, and periods) that matches an
already-extracted candidate tag gets a 1.5x score boost. It doesn't add a model call — it only
re-weights the same heuristic candidates.

`--tag-prompt` on `organize`/`preview` (below) is a distinct, LLM-facing prompt — see the next
