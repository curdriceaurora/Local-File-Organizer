...
This page captures the current beta1 runtime flow for the highest-churn subsystems referenced in issue `#1025`.
...
`PipelineOrchestrator` supports both the composable stage path and the legacy router-plus-processor path. The stage path is the canonical beta1 flow.
