...
class StorageReporter:
    ...
    def export_to_csv(self, duplicate_groups: list[dict[str, Any]], output_path: Path) -> None:
        ...
        """
        try:
            with open(output_path, "w", newline="", encoding="utf-8") as f:  # noqa: safedir-required, atomic-write  # dedup report writer — output path validated at CLI/API boundary
                writer = csv.writer(f)
    ...
    def export_to_json(self, duplicate_results: dict[str, Any], output_path: Path) -> None:
        ...
        """
        try:
            with open(output_path, "w", encoding="utf-8") as f:  # noqa: safedir-required, atomic-write  # dedup report writer — output path validated at CLI/API boundary
