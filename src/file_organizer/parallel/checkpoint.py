...
def compute_file_hash(path: Path) -> str:
    ...
    with open(path, "rb") as f:  # noqa: safedir-required, atomic-write  # checkpoint hash — path is a managed checkpoint file, content is read for integrity
        while True:
...
class CheckpointManager:
    ...
    def save_checkpoint(self, checkpoint: Checkpoint) -> None:
        ...
        try:
            with open(temp_path, "w", encoding="utf-8") as f:  # noqa: safedir-required, atomic-write  # checkpoint writer — path is an internally managed temp checkpoint file
