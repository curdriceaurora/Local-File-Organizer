...
class TestAppFactory:
    ...
    def test_injected_settings_are_honoured(self) -> None:
        """Settings passed to create_app must win over the loaded defaults.

        These tests previously patched ``file_organizer.api.main.ApiSettings``,
        which create_app never calls -- it uses ``load_settings()``. The patch
        was inert, so the whole class ran against real loaded settings while
        appearing to be isolated. Pin the injection so that cannot regress
        silently.
        """
        enabled = create_app(ApiSettings(environment="test", enable_docs=True))
        disabled = create_app(ApiSettings(environment="test", enable_docs=False))
