...
class TestRegisterUnregister:
    ...
    def test_register_and_retrieve(
        self, plugin_record_factory: Callable[[str], PluginRecord]
    ) -> None:
        reg = PluginRegistry()
        record = plugin_record_factory("alpha")
        reg._records["alpha"] = record
        assert reg.get_plugin("alpha") is record
        assert reg.list_plugins() == ["alpha"]

    def test_unregister(self, plugin_record_factory: Callable[[str], PluginRecord]) -> None:
        reg = PluginRegistry()
        record = plugin_record_factory("alpha")
        reg._records["alpha"] = record
    ...
    def test_list_plugins_sorted(
        ...
        for name in ["charlie", "alpha", "bravo"]:
            reg._records[name] = plugin_record_factory(name)
...
class TestCallAll:
    ...
    def test_call_all_collects_results(
        ...
        reg = PluginRegistry()
        rec_a = plugin_record_factory("a")
        rec_b = plugin_record_factory("b")
    ...
    def test_call_all_captures_exceptions(
        self, plugin_record_factory: Callable[[str], PluginRecord]
    ) -> None:
        reg = PluginRegistry()
        rec_ok = plugin_record_factory("ok")
        rec_bad = plugin_record_factory("bad")
    ...
    def test_call_all_passes_args_kwargs(
        self, plugin_record_factory: Callable[[str], PluginRecord]
    ) -> None:
        reg = PluginRegistry()
        rec = plugin_record_factory("p")
...
class TestUnloadPlugin:
    ...
    def test_unload_calls_on_unload_and_stop(
        self, plugin_record_factory: Callable[[str], PluginRecord]
    ) -> None:
        reg = PluginRegistry()
        record = plugin_record_factory("p")
    ...
    def test_unload_stops_executor_even_on_unload_error(
        self, plugin_record_factory: Callable[[str], PluginRecord]
    ) -> None:
...
class TestUnloadAll:
    ...
    def test_unload_all_clears_registry(
        self, plugin_record_factory: Callable[[str], PluginRecord]
    ) -> None:
        reg = PluginRegistry()
        for name in ["a", "b", "c"]:
            record = plugin_record_factory(name)
            reg._records[name] = record

        reg.unload_all()
        assert reg.list_plugins() == []

    def test_unload_all_continues_on_individual_failure(
        self, plugin_record_factory: Callable[[str], PluginRecord]
    ) -> None:
...
class TestLoadPlugin:
    ...
    def test_duplicate_name_raises(
        self,
        tmp_path: Path,
        plugin_record_factory: Callable[[str], PluginRecord],
        ...
        reg = PluginRegistry()
        reg._records["test-plugin"] = plugin_record_factory("test-plugin")
        plugin_dir = plugin_manifest_dir(
            tmp_path,
            {
                "name": "test-plugin",
                "version": "1.0.0",
