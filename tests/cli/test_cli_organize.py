...
class TestOrganize:
    ...
    def test_organize_text_only_alias_for_no_vision(
        ...
        """--text-only should be treated identically to --no-vision."""
        input_dir = tmp_path / "input"
        output_dir = tmp_path / "output"
        input_dir.mkdir()
        output_dir.mkdir()
    ...
    def test_organize_no_prefetch_flag_passes_through(
        ...
        """--no-prefetch should pass no_prefetch=True (backward compat alias)."""
        input_dir = tmp_path / "input"
        output_dir = tmp_path / "output"
        input_dir.mkdir()
        output_dir.mkdir()
    ...
    def test_organize_sequential_with_max_workers_one_is_valid(
        ...
        """--sequential combined with --max-workers 1 should succeed (not rejected)."""
        input_dir = tmp_path / "input"
        output_dir = tmp_path / "output"
        input_dir.mkdir()
        output_dir.mkdir()
    ...
    def test_organize_prefetch_depth_zero_explicit(
        ...
        """--prefetch-depth 0 should pass prefetch_depth=0 explicitly."""
        input_dir = tmp_path / "input"
        output_dir = tmp_path / "output"
        input_dir.mkdir()
        output_dir.mkdir()
