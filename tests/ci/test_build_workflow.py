...
class TestNoRustOrTauriSteps:
    ...
    def test_no_tauri_steps(self) -> None:
        ...
        raw = WORKFLOW_PATH.read_text(encoding="utf-8")
        assert "tauri" not in raw.lower() or "src-tauri" not in raw
...
class TestDesktopBuildStep:
    ...
    def test_cli_build_step_present(self) -> None:
        ...
        names = _build_step_names()
        assert any(
            ("CLI" in name or "executable" in name.lower() or "Build" in name)
            and "desktop" not in name.lower()
            for name in names
        ), f"No CLI build step found in: {names}"
...
class TestReleaseJob:
    def test_release_needs_build_only(self) -> None:
        ...
        if isinstance(needs, str):
            needs = [needs]
        assert "build" in needs, f"release job must need 'build', got: {needs}"
        assert "test-rust" not in needs, f"release job must NOT need 'test-rust', got: {needs}"
