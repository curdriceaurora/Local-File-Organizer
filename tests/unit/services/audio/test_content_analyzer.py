...
def test_analyze_empty():
    ...
    meta = AudioMetadata(file_path="dummy", file_size=1, format="mp3", duration=1, bitrate=1, sample_rate=1, channels=1)
...
def test_analyze_full():
    ...
    meta = AudioMetadata(
        file_path="dummy", file_size=1, format="mp3", duration=1, bitrate=1, sample_rate=1, channels=1,
        title="A beautiful software startup", comment="We use python and ai", genre="Tech", artist="John", album="Tech Pod"
    )
    ...
    transcription = TranscriptionResult(text="Hello. Hi there. Let's talk about machine learning.", segments=segments, language="en", language_confidence=1.0, duration=10.0, options={})
