...
Browser-based end-to-end tests live in `tests/playwright/` and use `@pytest.mark.playwright`. They exercise the desktop UI via a real Chromium browser and require the pywebview desktop app to be running. These tests are **not** part of the standard CI suite — run them manually when working on the desktop UI layer.
