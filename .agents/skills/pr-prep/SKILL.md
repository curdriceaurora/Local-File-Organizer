...
8. **Check internal docs for staleness**
   ...
   - `.Codex/rules/` files: any references to old patterns?
   - `memory/MEMORY.md`: any stale entries?

9. **Verify cross-references**
   - Do docs reference files that exist? Run `bash .Codex/scripts/pre-commit-validation.sh` (includes broken-link check)
...
10. **Run pre-commit validation**
   ```bash
   bash .Codex/scripts/pre-commit-validation.sh
   ```
...
| Doc search misses stale references | Grep too narrow (exact match only) | Use Agent tool for broad search across `docs/`, `CONTRIBUTING.md`, `README.md`, `.Codex/` |
