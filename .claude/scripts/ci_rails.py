...
Runs the rails declared in ``.claude/ci-rails.toml``. Each rail has a *mode*:
...
(exit 0): zero behavior change. See ``.claude/rules/ci-rails.md``.

Usage::

    python .claude/scripts/ci_rails.py              # respect each rail's mode
    python .claude/scripts/ci_rails.py --list       # list registered rails
    python .claude/scripts/ci_rails.py --enforce-all # treat every rail as enforce (CI)
    python .claude/scripts/ci_rails.py --registry PATH
"""
...
# .claude/scripts/ci_rails.py -> parent (scripts) -> parent (.claude) -> ci-rails.toml
DEFAULT_REGISTRY = Path(__file__).resolve().parent.parent / "ci-rails.toml"
